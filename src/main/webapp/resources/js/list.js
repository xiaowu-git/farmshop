...
$(document).ready(function() {
	...
	//获得文本框对象
    var pc_stock_str = $("#pc_stock").text();
    var h5_stock_str = $("#h5_stock").text();
	if(parseInt(pc_stock_str) <= 0 || parseInt(h5_stock_str) <= 0) {
        $("#text_box").val('0');
	}
	var t = $("#text_box");
	//初始化数量为1,并失效减
	...
	//数量增加操作
	$("#add").click(function() {
			if(parseInt(pc_stock_str) < parseInt(t.val()) + 1 || parseInt(h5_stock_str) < parseInt(t.val()) + 1) {
				t.val(parseInt(pc_stock_str))
			}else {
